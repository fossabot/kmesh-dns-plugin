 ...
 */
 ...
 type dnsProxy struct {
	 ...
	 protocol       string
	 resolver       *KmeshDNSServer
 }
 
 func newDNSProxy(protocol, addr string, resolver *KmeshDNSServer) (*dnsProxy, error) {
	 p := &dnsProxy{
	 ...
	 log.Debugf("Starting local %s DNS server on %v", p.protocol, addr)
	 if err != nil {
 ...
 func (p *dnsProxy) start(stop <-chan struct{}) {
	 err := p.server.ActivateAndServe()
	 if err != nil {
		 log.Errorf("Kmesh %s DNS server terminated: %v", p.protocol, err)
	 }
	 go func() {
		 <-stop
		 p.close()
